...
class GameState:
    ...
    def create_white_ball(self):
        ...
        if self.next_state_function:
            self.white_ball.move_to(self.state["white"])
        else:
    ...
    def game_event_handler(self, event): 
        if config.speculation and self.next_state_function:
        ...
        elif (not config.speculation) and (not self.next_state_function):
            if event.type == "POTTED":
                # print("Pot detected")
                self.table_coloring.update(self)
                self.balls.remove(event.data)
    ...
    def set_pool_balls(self, level=1):
        ...
        if not self.next_state_function:
            self.create_white_ball()
            ball_placement_sequence = list(range(1, config.total_ball_num[level]))
            random.shuffle(ball_placement_sequence)
            ball_config = config.level_config[level]

            for i in ball_placement_sequence:
                ball_iteration = ball.BallSprite(i)
                ball_iteration.move_to(ball_config[i])
                self.balls.add(ball_iteration)
            self.all_sprites.add(self.balls)
        else :
            self.create_white_ball()
            ball_list = copy.deepcopy(list(self.state.values()))
            ball_list.remove(self.state["white"])
            for k in self.state.keys():
                if self.state[k] != self.state["white"]:
                    ball_iteration = ball.BallSprite(k)
                    ball_iteration.move_to(self.state[k])
                    self.balls.add(ball_iteration)
            self.all_sprites.add(self.balls)
    ...
    def start_pool(self, level=1):
        self.reset_state()
        self.generate_table()
        self.set_pool_balls(level)
        self.cue = cue.Cue(self.white_ball, self.next_state_function)
        if not self.next_state_function:
            self.cue.agent.set_holes(copy.deepcopy(self.holes_x), copy.deepcopy(self.holes_y.copy()), radius=config.ball_radius)
    ...
    def redraw_all(self, update=True):
        ...
        if update and self.render:
            pygame.display.flip()
        self.mark_one_frame(self.render)
